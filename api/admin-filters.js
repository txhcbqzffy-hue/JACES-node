...
const ALLOWED_MENUS = new Set(['categories', 'nouveautes', 'accessoires', 'collaborations']);
...
module.exports = async function handler(req, res) {
  ...
  try {
    if (action === 'create') {
      ...
      const nextPosition = existing && existing.length ? Number(existing[0].position || 0) + 1 : 0;
    ...
    return res.status(400).json({ error: 'action invalide (attendu: create, delete)' });
  } catch (error) {
    return res.status(500).json({ error: error.message });
  }
};
